...
class MainActivity : AppCompatActivity() {
    ...
    private fun checkAllPermissions() {
        ...
        }else {// 2. 런타임 앱 권한이 모두 혀용되어 있는지 확인
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == PERMISSIONS_REQUEST_CODE && grantResults.size == REQUIRED_PERMISSIONS.size){
            ...
            if (checkResult){
                ...
                //퍼미션이 종료되었다면 앱 종료
                Toast.makeText(
                    this@MainActivity, "퍼미션이 거부되었습니다. 앱을 다시 실행하여 퍼미션을 허용해주세요",
                    Toast.LENGTH_LONG
                ).show()
                finish()
            }
    ...
    private fun showDialogForLocationServiceSetting(){
        // 먼저 ActivityResultLauncher 를 설정을 해줘야한다.
        // 이 런처를 이용하여 결괏값을 반환해야 하는 인텐트를 실행할수있다.
        getGPSPermissionLauncher = registerForActivityResult(
            ActivityResultContracts.StartActivityForResult()) { result ->
            //결괏값을 받았을 때 로직
            if (result.resultCode == Activity.RESULT_OK){
                //사용자가 GPS 를 활성화 시켰는지 확인
                if (isLocationServicesAvailable()){
            ...
            builder.setCancelable(true) // 다이얼로그 창 바깥 터치 시 창 닫침
